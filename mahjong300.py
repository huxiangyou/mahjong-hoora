...
def yaku_and_output(tehai9:list,hoorakei:list,did_tsumo:bool,is_karaten:bool,tsumohai:int,dahai:int,is_number_only:bool=False,has_koyaku:bool=False)->('hansuu',str):
	...
	string=""
	if dahai:
		string+=s.da+" ["+output_hai(dahai,is_number_only,jihai_in_kanji)+"] "
	if did_tsumo:
		string+=s.hoora+" | "
	elif did_tsumo==False:
		string+=(s.karaten if is_karaten else s.tenpai)+" ["+output_hai(tsumohai,is_number_only,jihai_in_kanji)+"] | "
	for j in hoorakei:
		string+=output_hais(j[1:],is_number_only,False," ")
	string+="|"
...
def main(tehai:str=None,tehai1:list=None,dahai:bool=False,is_number_only:bool=False,has_koyaku:bool=False)->str:
	string=""
	...
	#show tehai
	if not dahai:
		string+=s.tehai+s.colon+output_hais(tehai1,is_number_only,jihai_in_kanji,end="\n\n")

		if any(i>4 for i in tehai9):
			string+=s.more_than_4.format(s.ideographic_comma.join([output_hai(i,is_number_only,jihai_in_kanji) for i in range(10 if is_number_only else 38) if tehai9[i]>4]))+"\n"
	...
	if not dahai and is_tenpai and agarihais:
		...
		if not is_number_only:
			...
			for agarihai_han_i in range(len(agarihai_han)):
				string+=output_hai(agarihai_han[agarihai_han_i][0],is_number_only,jihai_in_kanji)+" "
			...
			for agarihai_i in agarihais:
				string+=output_hai(agarihai_i,is_number_only,jihai_in_kanji)+" "
		string+="\n"
...
def set_lang(event=None):
	global s
	global jihai_in_kanji
	if c_lang.get()=='English':
		s=en
		jihai_in_kanji=False
	elif c_lang.get()=='日本語':
		s=ja
		jihai_in_kanji=True
